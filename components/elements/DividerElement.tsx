...
const DividerElement: React.FC<DividerElementProps> = ({ content, id }) => {
  ...
  return (
    <div
      className={`w-full h-full flex items-center justify-center p-2 relative group cursor-pointer rounded-lg transition-all duration-200 ${
        isSelected ? "bg-accent/50 ring-2 ring-primary/30" : "hover:bg-accent/30"
      }`}
      onClick={(e) => {
        e.stopPropagation();
        dispatch(addElementClicked({ id: id, type: "divider" }));
      }}
    >
